	...
	widgets.tabs = function(elem){
		...
		this._observer = new window.MutationObserver(function(mutations){
			mutations.forEach(function(m,k){
				if (!m.addedNodes.length
				 && !m.removedNodes.length) {return false;}

				m.removedNodes.forEach(function(node){
					/* If we remove tabs we need to remove their content 
					 * if available */
					if (!node.classList.contains('tab')) {return;}
					if (node._content) {
						node._content.parentNode.removeChild(node._content);
					}
				});
		...
		this._observer.observe(this._container, { childList:true, subtree:true, attributes:false});
...
//alert(state);
	...
	widgets.tabs.prototype.update = function(e){
		...
		this.tmp = this._container.querySelectorAll('li.tab:not([data-tab])');
		Array.prototype.slice.call(this.tmp).forEach(function(tab,k){
			...
			if( isChecked && left + width > this._container.offsetWidth ){
				//FIXME: no
				//ths.panel.scrollLeft = (left+width)-this._container.offsetWidth;
			}
			...
			if (content) {
				...
				tab.addEventListener('click',function(e){
					...
					var change = Math.abs(this.panel.startX - x);
					if( change > 20 ){return false;}

					Array.prototype.slice.call(this.content.children).forEach(function(v,k){
						v.classList.remove('active');
					});
					Array.prototype.slice.call(tab.parentNode.children).forEach(function(v,k){
						v.classList.remove('active');
					});
